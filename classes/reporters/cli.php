...
class cli extends atoum\reporter
{
	...
	public function runnerStop(atoum\runner $runner)
	{
		...
		if ($errorNumber > 0)
		{
			...
			foreach ($score->getErrors() as $error)
			{
				...
				self::write(sprintf($this->locale->_('Error %s generated by file %s at line %d'), self::getErrorLabel($error['type']), $error['file'], $error['line']), 2);
	...
	public static function write($message = '', $level = 0)
	{
		...
		foreach ($messages as $message)
		{
			if ($message != '')
			{
				echo ($level <= 0 ? '' : str_repeat('   ', $level)) . trim($message) . PHP_EOL;
			}
		}
	}
}
