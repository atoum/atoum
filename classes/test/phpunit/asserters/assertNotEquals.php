...
class assertNotEquals extends asserter
{
	public function setWithArguments(array $arguments)
	{
		...
		$assertion = function($asserter, $actual, $expected) {
			$asserter->setWith($actual)->isNotEqualTo($expected);
		};
		...
		switch (true)
		{
			...
			case (is_float($arguments[1]) && is_float($arguments[0])) || (is_int($arguments[1]) && is_int($arguments[0])):
				...
				$assertion = function($asserter, $actual, $expected) use ($arguments) {
					...
					if ($exception === null)
					{
						$asserter->fail('%s is nearly equal to %s', $asserter->getAnalyzer()->getTypeOf($actual), $asserter->getAnalyzer()->getTypeOf($expected));
					}
			...
			case is_string($arguments[1]) || is_string($arguments[0]):
				...
				$assertion = function($asserter, $actual, $expected) use ($arguments) {
					$asserter->setWith((string) $actual)->isNotEqualTo((string) $expected, $arguments[3]);
				};
				break;
