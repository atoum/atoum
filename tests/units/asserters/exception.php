...
class exception extends atoum\test
{
	public function test__construct()
	{
		...
		$asserter = new asserters\exception($score, $locale);
	...
	public function testSetWith()
	{
		...
		$this->assert
			->exception($exception)
				...
				->hasMessage(sprintf($locale->_('%s is not an exception'), asserters\exception::toString($variable)))
		...
		try
		{
			$line = __LINE__; $this->assert->object($asserter->setWith(new \exception()))->isIdenticalTo($asserter);
		}
		catch (\exception $exception) {}
