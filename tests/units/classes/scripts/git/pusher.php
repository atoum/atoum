		...
		$this
			...
			->then
				...
				->mock($taggerEngine)
					...
					->call('tagVersion')
						->before($this->mock($git)
							->call('addAllAndCommit')->withArguments('Set version to 2.1.x-dev.')->once())
						->after($this->mock($taggerEngine)->call('setSrcDirectory')->withArguments($pusher->getWorkingDirectory())->once())
						->after($this->mock($taggerEngine)->call('setVersion')->withArguments('$Rev:' . ' 2.1.x-dev $')->once()) // Don't remove concatenation operator to avoid tagger replace the string.
							->once()
