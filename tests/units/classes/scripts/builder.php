...
class builder extends atoum\test
{
	...
	public function testCheckUnitTests()
	{
		$this
			...
			->mock($php)
				->beforeMethodCall('execute')
					->call('reset')->once()
					->call('addOption')->withArguments('-f', $workingDirectory . \DIRECTORY_SEPARATOR . $unitTestRunnerScript)->once()
					->call('addArgument')->withArguments('-ncc')->once()
					->call('addArgument')->withArguments('-d', $workingDirectory . \DIRECTORY_SEPARATOR . 'tests' . \DIRECTORY_SEPARATOR . 'units' . \DIRECTORY_SEPARATOR . 'classes')->once()
					->call('addArgument')->withArguments('-p', $php->getBinaryPath())->once()
					->call('addArgument')->withArguments('-sf', $scoreFile)->once()
					->call('addArgument')->withArguments('-c', $runnerConfigurationFile)->once()
	...
	public function testCreatePhar()
	{
		$this
			...
			->then
				...
				->mock($builder)
					->call('writeErrorInErrorsDirectory')->withArguments('Unable to execute ' . $php . ': ' . $stderr)->once()
				->mock($vcs)
