    ...
    public function testNotify()
    {
        $this
            ...
            ->then
                ...
                ->adapter($adapter)
                    ->call('system')->withArguments(sprintf('growlnotify --title %s --name atoum --message %s --image %s --url %s', escapeshellarg('Success!'), escapeshellarg($successString), escapeshellarg($image), escapeshellarg('')))->once()
            ...
            ->and($this->calling($score)->getincompletedMethodNumber = 1)
            ->and($field = new testedClass($adapter))
            ->and($field->setLocale($locale))
            ...
            ->then
                ...
                ->adapter($adapter)
                    ->call('system')->withArguments(sprintf('growlnotify --title %s --name atoum --message %s --image %s --url %s', escapeshellarg('Failure!'), escapeshellarg($failureString), escapeshellarg($image), escapeshellarg('')))->once()
            ...
            ->and($this->calling($score)->getincompletedMethodNumber = $incompletedTestNumber = rand(2, PHP_INT_MAX))
            ->and($field = new testedClass($adapter))
            ->and($field->setLocale($locale))
            ->and($field->setFailureImage($image = uniqid()))
            ->and($field->handleEvent(atoum\runner::runStop, $runner))
