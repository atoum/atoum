	...
	public function test__toString()
	{
		...
		$this->assert
			...
			->castToString($field->setWithRunner($runner))->isEqualTo(sprintf('There are %d errors:', sizeof($allErrors)) . PHP_EOL .
				...
				sprintf('Error %s in %s on line %d, generated by file %s on line %d:', $type, $file, $line, $errorFile, $errorLine) . PHP_EOL .
				$message . PHP_EOL .
				$otherClass . '::' . $otherMethod . '():' . PHP_EOL .
				sprintf('Error %s in %s on line %d, generated by file %s on line %d:', $otherType, $otherFile, $otherLine, $otherErrorFile, $otherErrorLine) . PHP_EOL .
		...
		$this->assert
			...
			->castToString($field->setWithRunner($runner))->isEqualTo(
				...
				sprintf(
					$locale->_('%s:'),
					$errorColorizer->colorize(sprintf($locale->_('Error %s in %s on line %d, generated by file %s on line %d'), $type, $file, $line, $errorFile, $errorLine))
				) .
				PHP_EOL .
				$message . PHP_EOL .
				$methodPrompt .
				...
				sprintf(
					$locale->_('%s:'),
					$errorColorizer->colorize(sprintf($locale->_('Error %s in %s on line %d, generated by file %s on line %d'), $otherType, $otherFile, $otherLine, $otherErrorFile, $otherErrorLine))
				) .
				PHP_EOL .
				$firstOtherMessage . PHP_EOL .
				$secondOtherMessage . PHP_EOL
		...
		$this->assert
			...
			->castToString($field->setWithRunner($runner))->isEqualTo(sprintf('There are %d errors:', sizeof($allErrors)) . PHP_EOL .
				$class . '::' . $method . '():' . PHP_EOL .
				sprintf('Error %s in %s on line %d, generated by file %s on line %d in case \'%s\':', $type, $file, $line, $errorFile, $errorLine, $case) . PHP_EOL .
				$message . PHP_EOL .
				$otherClass . '::' . $otherMethod . '():' . PHP_EOL .
				sprintf('Error %s in %s on line %d, generated by file %s on line %d in case \'%s\':', $otherType, $otherFile, $otherLine, $otherErrorFile, $otherErrorLine, $otherCase) . PHP_EOL .
				$firstOtherMessage . PHP_EOL .
				$secondOtherMessage . PHP_EOL
			)
		;
		...
		$this->assert
			...
			->castToString($field->setWithRunner($runner))->isEqualTo(
				...
				sprintf(
					$locale->_('%s:'),
					$errorColorizer->colorize(sprintf($locale->_('Error %s in %s on line %d, generated by file %s on line %d in case \'%s\''), $type, $file, $line, $errorFile, $errorLine, $case))
				) .
				PHP_EOL .
				$message . PHP_EOL .
				$methodPrompt .
				...
				sprintf(
					$locale->_('%s:'),
					$errorColorizer->colorize(sprintf($locale->_('Error %s in %s on line %d, generated by file %s on line %d in case \'%s\''), $otherType, $otherFile, $otherLine, $otherErrorFile, $otherErrorLine, $otherCase))
				) .
				PHP_EOL .
				$firstOtherMessage . PHP_EOL .
				$secondOtherMessage . PHP_EOL
		...
		$this->assert
			...
			->castToString($field->setWithRunner($runner))->isEqualTo(sprintf('There is %d error:', sizeof($allErrors)) . PHP_EOL .
				$class . '::' . $method . '():' . PHP_EOL .
				sprintf('Error %s in unknown file on unknown line, generated by file %s on line %d:', $type, $errorFile, $errorLine) . PHP_EOL .
				$message . PHP_EOL
			)
		;
		...
		$this->assert
			...
			->castToString($field->setWithRunner($runner))->isEqualTo(
				...
				sprintf(
					$locale->_('%s:'),
					$errorColorizer->colorize(sprintf($locale->_('Error %s in unknown file on unknown line, generated by file %s on line %d'), $type, $errorFile, $errorLine))
				) .
				PHP_EOL .
				$message . PHP_EOL
			)
		...
		$this->assert
			...
			->castToString($field->setWithRunner($runner))->isEqualTo(sprintf('There is %d error:', sizeof($allErrors)) . PHP_EOL .
				$class . '::' . $method . '():' . PHP_EOL .
				sprintf('Error %s in unknown file on unknown line, generated by file %s on line %d in case \'%s\':', $type, $errorFile, $errorLine, $case) . PHP_EOL .
				$message . PHP_EOL
			)
		;
		...
		$this->assert
			...
			->castToString($field->setWithRunner($runner))->isEqualTo(
				...
				sprintf(
					$locale->_('%s:'),
					$errorColorizer->colorize(sprintf($locale->_('Error %s in unknown file on unknown line, generated by file %s on line %d in case \'%s\''), $type, $errorFile, $errorLine, $case))
				) .
				PHP_EOL .
				$message . PHP_EOL
			)
		...
		$this->assert
			...
			->castToString($field->setWithRunner($runner))->isEqualTo(sprintf('There is %d error:', sizeof($allErrors)) . PHP_EOL .
				$class . '::' . $method . '():' . PHP_EOL .
				sprintf('Error %s in %s on unknown line, generated by file %s on line %d:', $type, $file, $errorFile, $errorLine) . PHP_EOL .
				$message . PHP_EOL
			)
		;
		...
		$this->assert
			...
			->castToString($field->setWithRunner($runner))->isEqualTo(
				...
				sprintf(
					$locale->_('%s:'),
					$errorColorizer->colorize(sprintf($locale->_('Error %s in %s on unknown line, generated by file %s on line %d'), $type, $file, $errorFile, $errorLine))
				) .
				PHP_EOL .
				$message . PHP_EOL
			)
		...
		$this->assert
			...
			->castToString($field->setWithRunner($runner))->isEqualTo(sprintf('There is %d error:', sizeof($allErrors)) . PHP_EOL .
				$class . '::' . $method . '():' . PHP_EOL .
				sprintf('Error %s in %s on unknown line, generated by file %s on line %d in case \'%s\':', $type, $file, $errorFile, $errorLine, $case) . PHP_EOL .
				$message . PHP_EOL
			)
		;
		...
		$this->assert
			...
			->castToString($field->setWithRunner($runner))->isEqualTo(
				...
				sprintf(
					$locale->_('%s:'),
					$errorColorizer->colorize(sprintf($locale->_('Error %s in %s on unknown line, generated by file %s on line %d in case \'%s\''), $type, $file, $errorFile, $errorLine, $case))
				) .
				PHP_EOL .
				$message . PHP_EOL
			)
