...
class cli extends units\report\fields\runner
{
	...
	public function test__toString()
	{
		...
		$defaultField = new runner\errors\cli();
		$customField = new runner\errors\cli($titlePrompt = new prompt(uniqid()), $titleColorizer = new colorizer(uniqid(), uniqid()), $methodPrompt = new prompt(uniqid()), $methodColorizer = new colorizer(uniqid(), uniqid()), $errorPrompt = new prompt(uniqid()), $errorColorizer = new colorizer(uniqid(), uniqid()), $locale = new atoum\locale());

		$score->getMockController()->getErrors = array();

		$this->assert('when there is no error')
		...
		$this->assert('when there is errors with file, line and no case')
			->castToString($defaultField-> setWithRunner($runner))->isEqualTo(sprintf('There are %d errors:', sizeof($allErrors)) . PHP_EOL .
		...
		$this->assert('when there is errors with file, line and case')
			->castToString($defaultField-> setWithRunner($runner))->isEqualTo(sprintf('There are %d errors:', sizeof($allErrors)) . PHP_EOL .
				$class . '::' . $method . '():' . PHP_EOL .
				sprintf('Error %s in %s on line %d, generated by file %s on line %d %s:', strtoupper($type), $file, $line, $errorFile, $errorLine, $case) . PHP_EOL .
				$message . PHP_EOL .
				$otherClass . '::' . $otherMethod . '():' . PHP_EOL .
				sprintf('Error %s in %s on line %d, generated by file %s on line %d %s:', strtoupper($otherType), $otherFile, $otherLine, $otherErrorFile, $otherErrorLine, $otherCase) . PHP_EOL .
				$firstOtherMessage . PHP_EOL .
				$secondOtherMessage . PHP_EOL
			)
			->castToString($customField-> setWithRunner($runner))->isEqualTo(
				...
				sprintf(
					$locale->_('%s:'),
					$errorColorizer->colorize(sprintf($locale->_('Error %s in %s on line %d, generated by file %s on line %d %s'), strtoupper($type), $file, $line, $errorFile, $errorLine, $case))
				) .
				PHP_EOL .
				$message . PHP_EOL .
				$methodPrompt .
				...
				sprintf(
					$locale->_('%s:'),
					$errorColorizer->colorize(sprintf($locale->_('Error %s in %s on line %d, generated by file %s on line %d %s'), strtoupper($otherType), $otherFile, $otherLine, $otherErrorFile, $otherErrorLine, $otherCase))
				) .
				PHP_EOL .
				$firstOtherMessage . PHP_EOL .
				$secondOtherMessage . PHP_EOL
		...
		$this->assert('when there is errors with no file, no line and no case')
			->castToString($defaultField-> setWithRunner($runner))->isEqualTo(sprintf('There is %d error:', sizeof($allErrors)) . PHP_EOL .
				$class . '::' . $method . '():' . PHP_EOL .
		...
		$this->assert('when there is errors with no file, no line and case')
			->castToString($defaultField-> setWithRunner($runner))->isEqualTo(sprintf('There is %d error:', sizeof($allErrors)) . PHP_EOL .
				$class . '::' . $method . '():' . PHP_EOL .
				sprintf('Error %s in unknown file on unknown line, generated by file %s on line %d %s:', strtoupper($type), $errorFile, $errorLine, $case) . PHP_EOL .
				$message . PHP_EOL
			)
			->castToString($customField-> setWithRunner($runner))->isEqualTo(
				$titlePrompt .
				...
				sprintf(
					$locale->_('%s:'),
					$errorColorizer->colorize(sprintf($locale->_('Error %s in unknown file on unknown line, generated by file %s on line %d %s'), strtoupper($type), $errorFile, $errorLine, $case))
				) .
				PHP_EOL .
				$message . PHP_EOL
			)
		...
		$this->assert('when there is errors with no file, line and no case')
			->castToString($defaultField-> setWithRunner($runner))->isEqualTo(sprintf('There is %d error:', sizeof($allErrors)) . PHP_EOL .
				$class . '::' . $method . '():' . PHP_EOL .
				sprintf('Error %s in unknown file on unknown line, generated by file %s on line %d:', strtoupper($type), $errorFile, $errorLine) . PHP_EOL .
				$message . PHP_EOL
		...
		$this->assert('when there is errors with file, no line, no error file, no error line and a case')
			->castToString($defaultField-> setWithRunner($runner))->isEqualTo(sprintf('There is %d error:', sizeof($allErrors)) . PHP_EOL .
				$class . '::' . $method . '():' . PHP_EOL .
				sprintf('Error %s in %s on unknown line, generated by unknown file %s:', strtoupper($type), $file, $case) . PHP_EOL .
				$message . PHP_EOL
			)
			->castToString($customField-> setWithRunner($runner))->isEqualTo(
				$titlePrompt .
				...
				sprintf(
					$locale->_('%s:'),
					$errorColorizer->colorize(sprintf($locale->_('Error %s in %s on unknown line, generated by unknown file %s'), strtoupper($type), $file, $case))
				) .
				PHP_EOL .
				$message . PHP_EOL
			)
		...
		$this->assert('when there is errors with no file, line and case')
			->castToString($defaultField-> setWithRunner($runner))->isEqualTo(sprintf('There is %d error:', sizeof($allErrors)) . PHP_EOL .
				$class . '::' . $method . '():' . PHP_EOL .
				sprintf('Error %s in unknown file on unknown line, generated by file %s on line %d %s:', strtoupper($type), $errorFile, $errorLine, $case) . PHP_EOL .
				$message . PHP_EOL
		...
		$this->assert('when there is errors with file, no line and no case')
			->castToString($defaultField-> setWithRunner($runner))->isEqualTo(sprintf('There is %d error:', sizeof($allErrors)) . PHP_EOL .
				$class . '::' . $method . '():' . PHP_EOL .
		...
		$this->assert('when there is errors with file, no line and case')
			->castToString($defaultField-> setWithRunner($runner))->isEqualTo(sprintf('There is %d error:', sizeof($allErrors)) . PHP_EOL .
				$class . '::' . $method . '():' . PHP_EOL .
				sprintf('Error %s in %s on unknown line, generated by file %s on line %d %s:', strtoupper($type), $file, $errorFile, $errorLine, $case) . PHP_EOL .
				$message . PHP_EOL
			)
			->castToString($customField-> setWithRunner($runner))->isEqualTo(
				$titlePrompt .
				...
				sprintf(
					$locale->_('%s:'),
					$errorColorizer->colorize(sprintf($locale->_('Error %s in %s on unknown line, generated by file %s on line %d %s'), strtoupper($type), $file, $errorFile, $errorLine, $case))
				) .
				PHP_EOL .
				$message . PHP_EOL
			)
