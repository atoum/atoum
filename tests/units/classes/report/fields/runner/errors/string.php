	...
	public function test__toString()
	{
		...
		$this->assert
			...
			->castToString($errors->setWithRunner($runner))->isEqualTo(runner\errors\string::titlePrompt . sprintf($locale->__('There is %d error:', 'There are %d errors:', sizeof($allErrors)), sizeof($allErrors)) . PHP_EOL .
				...
				runner\errors\string::errorPrompt . sprintf($locale->_('Error %s in %s on line %d:'), $type, $file, $line) . PHP_EOL .
				$message . PHP_EOL .
				runner\errors\string::methodPrompt . $otherClass . '::' . $otherMethod . '():' . PHP_EOL .
				runner\errors\string::errorPrompt . sprintf($locale->_('Error %s in %s on line %d:'), $otherType, $otherFile, $otherLine) . PHP_EOL .
		...
		$this->assert
			...
			->castToString($errors->setWithRunner($runner))->isEqualTo(runner\errors\string::titlePrompt . sprintf($locale->__('There is %d error:', 'There are %d errors:', sizeof($allErrors)), sizeof($allErrors)) . PHP_EOL .
				runner\errors\string::methodPrompt . $class . '::' . $method . '():' . PHP_EOL .
				runner\errors\string::errorPrompt . sprintf($locale->_('Error %s in unknown file on unknown line:'), $type) . PHP_EOL .
				$message . PHP_EOL
			)
		;
		...
		$this->assert
			...
			->castToString($errors->setWithRunner($runner))->isEqualTo(runner\errors\string::titlePrompt . sprintf($locale->__('There is %d error:', 'There are %d errors:', sizeof($allErrors)), sizeof($allErrors)) . PHP_EOL .
				runner\errors\string::methodPrompt . $class . '::' . $method . '():' . PHP_EOL .
				runner\errors\string::errorPrompt . sprintf($locale->_('Error %s in unknown file on line %d:'), $type, $line) . PHP_EOL .
				$message . PHP_EOL
			)
		;
		...
		$this->assert
			...
			->castToString($errors->setWithRunner($runner))->isEqualTo(runner\errors\string::titlePrompt . sprintf($locale->__('There is %d error:', 'There are %d errors:', sizeof($allErrors)), sizeof($allErrors)) . PHP_EOL .
				runner\errors\string::methodPrompt . $class . '::' . $method . '():' . PHP_EOL .
				runner\errors\string::errorPrompt . sprintf($locale->_('Error %s in %s on unknown line:'), $type, $file) . PHP_EOL .
				$message . PHP_EOL
			)
		;
	}
