		...
		$this
			...
			->then
				...
				->mock($factory)->call('build')
					->withArguments('recursiveDirectoryIterator', array($path), $iterator)->once()
					->withArguments('mageekguy\atoum\iterators\filters\recursives\dot', array($recursiveDirectoryIterator), $iterator)->once()
					->withArguments('mageekguy\atoum\iterators\filters\recursives\extension', array(new atoum\iterators\filters\recursives\dot($recursiveDirectoryIterator), array('php')), $iterator)->once()
			->if($iterator->acceptDots())
			->then
				->object($filterIterator = $iterator->getIterator())->isInstanceOf('mageekguy\atoum\iterators\filters\recursives\extension')
				->object($filterIterator->getInnerIterator())->isIdenticalTo($recursiveDirectoryIterator)
				->mock($factory)->call('build')
					->withArguments('recursiveDirectoryIterator', array($path), $iterator)->exactly(2)
					->withArguments('mageekguy\atoum\iterators\filters\recursives\extension', array($recursiveDirectoryIterator, array('php')), $iterator)->once()
			...
			->then
				...
				->object($filterIterator->getInnerIterator())->isIdenticalTo($recursiveDirectoryIterator)
				->mock($factory)->call('build')
					->withArguments('recursiveDirectoryIterator', array($path), $iterator)->exactly(3)
			->if($iterator->acceptDots())
			->and($iterator->acceptExtensions(array()))
			->then
				->object($iterator->getIterator())->isIdenticalTo($recursiveDirectoryIterator)
				->mock($factory)->call('build')
					->withArguments('recursiveDirectoryIterator', array($path), $iterator)->exactly(4)
