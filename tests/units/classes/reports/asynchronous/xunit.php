...
class xunit extends atoum\test
{
	...
	public function testBuild()
	{
		$this
			...
			->and($testScore->addDuration($file = 'file', $class, $method, $duration = 1))
			->and($testScore->addUncompletedMethod($file, $class, $otherMethod, $exitCode = 1, $output = 'output'))
			->and($testScore->addSkippedMethod($file, $class, $thirdMethod, $line = rand(1, PHP_INT_MAX), $message = 'message'))
			...
			->and($testScore->addFail($otherFile = 'otherFile', $otherClass, $otherMethod, 1, $asserter = 'asserter', $reason = 'reason'))
			->and($exception = new \mock\Exception())
			->and($exception->getMockController()->__toString = $trace = 'trace')
			->and($testScore->addException($otherFile, $otherClass, $thirdMethod, 1, $exception))
			->and($score->merge($testScore))
			->and($report->handleEvent(atoum\test::afterTestMethod, $test))
			->and($report->handleEvent(atoum\test::afterTestMethod, $test))
			->and($report->handleEvent(atoum\runner::runStop, $runner))
			...
			->and($testScore->addError($thirdFile = 'thirdFile', $thirdClassFqn, $fourthMethod, rand(0, PHP_INT_MAX), $type = E_ERROR, $message))
			->and($score->merge($testScore))
			->and($report->handleEvent(atoum\test::afterTestMethod, $test))
			->and($report->handleEvent(atoum\runner::runStop, $runner))
			->then
